# triangle_bybit_testnet_bot.py — торговый бот для тестовой сети Bybit
...
async def safety_net(currency):
    """Продает оставшиеся средства в тестовой сети"""
    if TESTNET_MODE:
        return True, "Testnet: skip recovery"
    
    try:
        balance = (await fetch_balances()).get(currency, 0)
        if balance <= 0:
            return True, "No balance to recover"
        
        symbol = f"{currency}/USDT" if f"{currency}/USDT" in exchange.symbols else f"USDT/{currency}"
        side = 'sell' if f"{currency}/USDT" in exchange.symbols else 'buy'
        
        formatted_amount = float(exchange.amount_to_precision(symbol, balance))
        order = await exchange.create_order(
            symbol=symbol,
            type='market',
            side=side,
...
async def verify_prices(steps, expected_prices):
    """Проверяет текущие цены на проскальзывание"""
    try:
        for i, (symbol, _, _) in enumerate(steps):
            ticker = await exchange.fetch_ticker(symbol)
            current_price = ticker['last']
            expected = expected_prices[i]
            
            deviation = abs(current_price / expected - 1)
            if deviation > MAX_SLIPPAGE:
                return False, f"Проскальзывание: {deviation*100:.2f}% на {symbol}"
        
        return True, "Цены подтверждены"
    except Exception as e:
        return False, str(e)
...
async def check_triangle(base, mid1, mid2, symbols, markets):
    try:
        ...
        if execute:
            ...
            # В тестовом режиме пропускаем проверку проскальзывания
            if not TESTNET_MODE:
                price_ok, price_msg = await verify_prices(steps, expected_prices)
                if not price_ok:
                    msg = f"⚠️ <b>Отмена сделки</b>\n{price_msg}\nМаршрут: {route_id}"
                    await send_telegram_message(msg)
                    log_trade(base, mid1, mid2, profit_percent, min_liquidity, "cancelled", price_msg)
                    return
...
async def main():
    try:
        ...
        if DEBUG_MODE:
            print(f"Подключено к тестовой сети Bybit: {status['status']}")
